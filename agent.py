...
CORRECTION_REQUEST_TEXT = (
    'Help us catch transcription errors. Reply to this email. '
    'In the body of your reply email, start with the word "correction:", then type the actual transcript word '
...
def record_transcript_email_sent(episode, message_id, recipients):
    append_jsonl(
        TRANSCRIPT_EMAIL_SENT_LOG_FILE,
        {
...
def format_transcript_html(episode, final_text, drawing_src=None, footer_src=None, correction_request_text=None):
    ...
    correction_request_html = ""
    if correction_request_text:
        correction_request_html = f'''
      <div class="corrections-note">{html.escape(correction_request_text)}</div>'''
...
<head>
  ...
  <style>
    ...
    .corrections-note {{ margin-top: 28px; padding: 16px 18px; background: #f5f5f0; border-left: 4px solid #7b9cdf; font-family: -apple-system, BlinkMacSystemFont, "Helvetica Neue", sans-serif; font-size: 14px; line-height: 1.65; color: #4b4b57; }}
...
"""
    ...
    html_body = format_transcript_html(
        ...
        correction_request_text=CORRECTION_REQUEST_TEXT,
    )
    ...
    message_id = make_msgid(domain="transcripts.local")
    msg["From"] = gmail_address
    msg["To"] = ", ".join(recipients)
    msg["Subject"] = subject
    msg["Message-ID"] = message_id
    msg["X-Transcript-Podcast"] = episode["podcast"]
    msg["X-Transcript-Episode"] = episode["title"]
    msg["X-Transcript-Published"] = episode.get("published", "")
    ...
    try:
        ...
        record_transcript_email_sent(episode, message_id, recipients)
        elapsed = time.time() - t0
